...
if(!file.exists(here("Project 2 BIOS","tuesdata_rainfall.RDS"))){
  tuesdata <- tidytuesdayR::tt_load('2020-01-07')
  rainfall <- tuesdata$rainfall
  temperature <- tuesdata$temperature
  
  # save the files to RDS objects
  saveRDS(tuesdata$rainfall, file= here("Project 2 BIOS","tuesdata_rainfall.RDS"))
  saveRDS(tuesdata$temperature, file=here("Project 2 BIOS","tuesdata_temperature.RDS"))
}


temperature <- readRDS(here("tuesdata_temperature.RDS"))

rainfall <- readRDS("tuesdata_rainfall.RDS")
...
plot1 <- df %>% filter(year >= 2014) %>% ggplot(aes(x = date)) + 
  ...
  labs(title = "Temperature trends since 2014", subtitle = "Seasonal trends in 6 Australian cities are consistent over time. The most variable temperatures are those in Perth and Melbourne.", caption = "Temperatures are shown for 5 Australian cities: Brisbane, Canberra, Melbourne, Perth, and Sydney. The Y axes are scaled the same to make comparisons across cities easier.",  x = "Date of measurement", y = "Temperature in Celcius") + 
...
rainplot <- rain_df %>% ggplot(aes(x = year, y = mean)) + 
  ...
  labs(title = "Rainfall in Australian Cities Since 2014", y = "Mean Rainfall (mm)", x = "Year", subtitle = str_wrap("Figure 2: Perth experiences the highest levels of rainfall anually. Canberra experiences the lowest levels of rainfall anually. 2015 was an unusually rainy year for Brisbane.", 100)) +
